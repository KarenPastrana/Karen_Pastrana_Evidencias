...
void loop() {
  ...
  //Parte de motores
  // Move each motor 20 steps
  stepper.move(20);
  stepper2.move(20);
  // Run to the target position for each motor
  while (stepper.distanceToGo() != 0 || stepper2.distanceToGo() != 0) {
    stepper.run();
    stepper2.run();
  }
  // Mueve cada motor 20 pasos con un retardo de 1000 microsegundos entre pasos
  stepMotor(MOTOR3_DIR_PIN, MOTOR3_STEP_PIN, 20, 1000);
  stepMotor(MOTOR4_DIR_PIN, MOTOR4_STEP_PIN, 20, 1000);
  
  // Retardo de 1 segundo antes de volver a mover
